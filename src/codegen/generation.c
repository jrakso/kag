#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>

#include "generation.h"
#include "vartable.h"

typedef struct Scope {
    VariableTable vars;
    size_t base_stack_size;
    struct Scope *parent;
} Scope;

typedef struct {
    const NodeProg *prog;
    StringBuilder text;  // instructions
    StringBuilder data;
    size_t stack_size;
    size_t label_count;
    Scope *current_scope;
    int nested_level;
} Generator;

static void gen_expr(Generator *g, const NodeExpr *expr);
static void gen_stmt(Generator *g, const NodeStmt *stmt);
static void begin_scope(Generator *g);
static void end_scope(Generator *g);

/* ------------------------------------------------ */
/* Generator helpers */
/* ------------------------------------------------ */

static void push(Generator *g, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    sb_append(&g->text, "\tpush ");
    sb_append_fmtv(&g->text, fmt, args);
    sb_append(&g->text, "\n");
    va_end(args);
    g->stack_size++;
}

static void pop(Generator *g, const char *reg) {
    sb_append_fmt(&g->text, "\tpop %s\n", reg);
    g->stack_size--;
}

static char *create_label(size_t label_id) {
    int len = snprintf(NULL, 0, "label%zu", label_id);
    if (len < 0) {
        fprintf(stderr, "snprintf formatting error\n");
        exit(EXIT_FAILURE);
    }
    char *label = malloc(len + 1);
    if (!label) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
    snprintf(label, len + 1, "label%zu", label_id);
    return label;  // Caller frees
}

static const Variable *lookup_var(Generator *g, const char *name) {
    Scope *s = g->current_scope;
    while (s) {
        Variable *var = var_table_find(&s->vars, name);
        if (var)
            return var;
        s = s->parent;
    }
    return NULL;
}

static void generator_init(Generator *g, const NodeProg *prog) {
    g->prog = prog;
    sb_init(&g->text);
    sb_init(&g->data);
    g->stack_size = 0;
    g->label_count = 0;
    g->current_scope = NULL;  // Important
    begin_scope(g);
    g->nested_level = 0;
}

static void generator_free(Generator *g) {
    while (g->current_scope) {
        end_scope(g);
    }
    sb_free(&g->data);
    sb_free(&g->text);
}

/* ------------------------------------------------ */
/* Term generation */
/* ------------------------------------------------ */

static void gen_term(Generator *g, const NodeTerm *term) {
    switch (term->type) {      
        case TERM_INT_LIT: {
            sb_append_fmt(&g->text, "\tmov rax, %s\n", term->data.int_lit->int_lit.value);
            push(g, "rax");
            break;
        }
        case TERM_STRING_LIT: {
            char *label = create_label(g->label_count++);
            // store string in data section
            sb_append_fmt(&g->data, "%s: db \"%s\", 0\n", label, term->data.string_lit->string_lit.value);
            // push pointer
            sb_append_fmt(&g->text, "\tlea rax, [rel %s]\n", label);
            push(g, "rax");
            free(label);
            break;
        }
        case TERM_IDENT: {
            const Variable *var = lookup_var(g, term->data.ident->ident.value);
            if (!var) {
                fprintf(stderr, "Undeclared identifier: %s\n", term->data.ident->ident.value);
                exit(EXIT_FAILURE);
            }
            push(g, "QWORD [rsp + %zu]", (g->stack_size - var->stack_loc - 1)*8);
            break;
        }
        case TERM_PAREN: {
            gen_expr(g, term->data.paren->expr);
            break;
        }
        default:
            break;
    }
}

/* ------------------------------------------------ */
/* Binary expression generation */
/* ------------------------------------------------ */

static void gen_bin_expr(Generator *g, const NodeBinExpr *bin_expr) {
    switch (bin_expr->type) {
        case BIN_ADD:
            gen_expr(g, bin_expr->data.add->rhs);
            gen_expr(g, bin_expr->data.add->lhs);
            pop(g, "rax");
            pop(g, "rbx");
            sb_append(&g->text, "\tadd rax, rbx\n");
            push(g, "rax");
            break;
        case BIN_SUB:
            gen_expr(g, bin_expr->data.sub->rhs);
            gen_expr(g, bin_expr->data.sub->lhs);
            pop(g, "rax");
            pop(g, "rbx");
            sb_append(&g->text, "\tsub rax, rbx\n");
            push(g, "rax");
            break;
        case BIN_MULTI:
            gen_expr(g, bin_expr->data.multi->rhs);
            gen_expr(g, bin_expr->data.multi->lhs);
            pop(g, "rax");
            pop(g, "rbx");
            sb_append(&g->text, "\tmul rbx\n");
            push(g, "rax");
            break;
        case BIN_DIV:
            gen_expr(g, bin_expr->data.div->rhs);
            gen_expr(g, bin_expr->data.div->lhs);
            pop(g, "rax");
            pop(g, "rbx");
            sb_append(&g->text, "\tdiv rbx\n");
            push(g, "rax");
            break;
        default: 
            break;
    }
}

/* ------------------------------------------------ */
/* Expression generation */
/* ------------------------------------------------ */

static void gen_expr(Generator *g, const NodeExpr *expr) {
    switch (expr->type) {
        case EXPR_TERM:
            gen_term(g, expr->data.term);
            break;
        case EXPR_BIN:
            gen_bin_expr(g, expr->data.bin);
            break;
        default: 
            break;
    }
}

/* ------------------------------------------------ */
/* Scope generation */
/* ------------------------------------------------ */

static void begin_scope(Generator *g) {
    Scope *s = malloc(sizeof(Scope));
    if (!s) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
    var_table_init(&s->vars);
    s->base_stack_size = g->stack_size;
    s->parent = g->current_scope;
    g->current_scope = s;
}

static void end_scope(Generator *g) {
    Scope *s = g->current_scope;
    size_t locals = g->stack_size - s->base_stack_size;
    if (locals > 0) {
        sb_append_fmt(&g->text, "\tadd rsp, %zu\n", locals * 8);
        g->stack_size = s->base_stack_size;
    }
    g->current_scope = s->parent;
    var_table_free(&s->vars);
    free(s);
}

static void gen_scope(Generator *g, const NodeScope *scope) {
    begin_scope(g);
    for (size_t i = 0; i < scope->size; i++) {
        gen_stmt(g, scope->stmts[i]);
    }
    end_scope(g);
}

/* ------------------------------------------------ */
/* If chain generation */
/* ------------------------------------------------ */

static void gen_if_chain(Generator *g, const NodeIfPred *pred, const char *end_label, size_t end_label_id) {
    if (!pred) {
        return;
    }
    switch (pred->type) {
        case IFPRED_ELIF: {
            gen_expr(g, pred->data.elif->expr);
            pop(g, "rax");
            char *label = create_label(g->label_count++);
            sb_append(&g->text, "\ttest rax, rax\n");
            sb_append_fmt(&g->text, "\tjz %s", label);
            sb_append_fmt(&g->text, " ; %d level elif - end label (%zu)\n", g->nested_level - 1, end_label_id);
            sb_append_fmt(&g->text, "\t; %d level elif (true)\n", g->nested_level - 1);
            gen_scope(g, pred->data.elif->scope);
            sb_append_fmt(&g->text, "\tjmp %s\n", end_label);
            sb_append_fmt(&g->text, "\t; %d level elif (false)\n", g->nested_level - 1);
            sb_append_fmt(&g->text, "%s:\n", label);
            if (pred->data.elif->pred) {
                gen_if_chain(g, pred->data.elif->pred, end_label, end_label_id);
            }
            free(label);
            break;
        }
        case IFPRED_ELSE: {
            int level = g->nested_level - 1;
            sb_append_fmt(&g->text, "\t; %d level else\n", level);      
            gen_scope(g, pred->data.else_->scope);
            break;
        }
        default: {
            break;
        }
    } 
}

/* ------------------------------------------------ */
/* Statement generation */
/* ------------------------------------------------ */

static void gen_stmt(Generator *g, const NodeStmt *stmt) {
    switch (stmt->type) {
        case STMT_EXIT: {
            gen_expr(g, stmt->data.exit->expr);
            sb_append(&g->text, "\tmov rax, 60\n");
            pop(g, "rdi");
            sb_append(&g->text, "\tsyscall\n");
            break;
        }
        case STMT_LET: {
            if (var_table_contains(&g->current_scope->vars, stmt->data.let->ident.value)) {
                fprintf(stderr, "Identifier already used: %s\n", stmt->data.let->ident.value);
                exit(EXIT_FAILURE);
            }
            var_table_append(&g->current_scope->vars, stmt->data.let->ident.value, g->stack_size);
            gen_expr(g, stmt->data.let->expr);
            break;
        }
        case STMT_ASSIGN: {
            const Variable *var = lookup_var(g, stmt->data.assign->ident.value);
            if (!var) {
                fprintf(stderr, "Undeclared identifier: %s\n", stmt->data.assign->ident.value);
                exit(EXIT_FAILURE);
            }
            gen_expr(g, stmt->data.assign->expr);
            pop(g, "rax");
            sb_append_fmt(&g->text, "\tmov [rsp + %zu], rax\n", (g->stack_size - var->stack_loc - 1)*8);
            break;
        }
        case STMT_SCOPE: {
            gen_scope(g, stmt->data.scope);
            break;
        }
        case STMT_IF: {
            int level = g->nested_level;
            g->nested_level++;
            char *label = create_label(g->label_count++);
            gen_expr(g, stmt->data.if_->expr);
            pop(g, "rax");
            sb_append(&g->text, "\ttest rax, rax\n");
            sb_append_fmt(&g->text, "\tjz %s", label);
            char *end_label = create_label(g->label_count++);
            size_t end_label_id = g->label_count - 1;
            sb_append_fmt(&g->text, " ; %d level if - end label (%zu)\n", level, end_label_id);
            sb_append_fmt(&g->text, "\t; %d level if (true)\n", level);
            gen_scope(g, stmt->data.if_->scope);
            sb_append_fmt(&g->text, "\tjmp %s\n", end_label);
            sb_append_fmt(&g->text, "\t; %d level if (false)\n", level);
            sb_append_fmt(&g->text, "%s:\n", label);
            if (stmt->data.if_->pred) {
                gen_if_chain(g, stmt->data.if_->pred, end_label, end_label_id);
            }
            sb_append_fmt(&g->text, "%s:\n", end_label);
            g->nested_level--;
            free(end_label);
            free(label);
            break;
        }
        default: {
            break;
        }
    }
}

/* ------------------------------------------------ */
/* Program generation */
/* ------------------------------------------------ */

static void gen_prog(Generator *g) {
    for (size_t i = 0; i < g->prog->size; i++) {
        gen_stmt(g, g->prog->stmts[i]);
    }
    // Default exit
    sb_append(&g->text,
        "\tmov rax, 60\n"
        "\tmov rdi, 0\n"
        "\tsyscall\n");
}

/* ------------------------------------------------ */
/* Generator API */
/* ------------------------------------------------ */

StringBuilder generate(const NodeProg *prog) {
    Generator g;
    generator_init(&g, prog);
    gen_prog(&g);
    StringBuilder sb;
    sb_init(&sb);
    sb_append(&sb, "section .data\n");
    sb_append(&sb, g.data.data);
    sb_append(&sb, "\nsection .text\n");
    sb_append(&sb, "global _start\n_start:\n");    
    sb_append(&sb, g.text.data);
    generator_free(&g);
    return sb;
}
